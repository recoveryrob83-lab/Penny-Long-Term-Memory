"""Ingest one immutable owning-HQ review receipt into its existing execution row."""
...
RECEIPT_PREFIX = "LIFEOS_HQ_REVIEW_INGESTION_RECEIPT="
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        service = WorkerHqReviewService(repository_root, database_path)
        receipt = service.ingest_review(args.run_id)
    ...
    payload = {
        ...
        "hq_review": service.status(limit=100),
        "verification": service.verification.status(limit=100),
    }
    print("HQ_REVIEW_INGESTION_OK")
