...
Git evidence references, canonical department project roots, and automatic HQ wakes
with the existing send-budget and safety-pause controls.
"""
...
def _scope_allows(allowed: object, actual: object) -> bool:
    ...
    return bool(
        allowed_path
        and actual_path
        and (actual_path == allowed_path or actual_path.startswith(allowed_path + "/"))
    )
...
def _parse_evidence_reference(reference: object) -> tuple[str, str | None, str | None]:
    ...
    if not separator or not path or not witness:
        raise WorkerRuntimeError(f"Evidence reference lacks path correlation: {text}")
    if witness == "preflight:not-found":
        return path, None, None
    if _HEX40.fullmatch(witness):
        return path, None, witness
    if witness.startswith("blob:") and _HEX40.fullmatch(witness.removeprefix("blob:")):
        return path, None, witness.removeprefix("blob:")
...
def _verify_git_witness(
    ...
    *,
    ...
    try:
        if commit_sha is not None and ingester._git("cat-file", "-t", commit_sha) != "commit":
            raise WorkerRuntimeError(f"Evidence witness is not a Git commit: {reference}")
        if blob_sha is not None and ingester._git("cat-file", "-t", blob_sha) != "blob":
            raise WorkerRuntimeError(f"Evidence witness is not a Git blob: {reference}")
    except WorkerRuntimeError as exc:
        if "is not a Git" in str(exc):
            raise
        raise WorkerRuntimeError(f"Evidence Git object is unavailable: {reference}") from exc
...
def _install_result_validation() -> None:
    ...
    def validate_report_correlation(
        ...
        actual_tools = {str(item) for item in payload.get("actual_tools") or ()}
        allowed_reads = (*advisory.requested_read_scopes, *advisory.requested_write_scopes)

        unauthorized_reads = _unauthorized_scopes(allowed_reads, actual_reads)
        unauthorized_writes = _unauthorized_scopes(advisory.requested_write_scopes, actual_writes)
        ...
        if payload.get("controlled_outcome") == "IMPLEMENT":
            missing_reads = _uncovered_scopes(advisory.requested_read_scopes, actual_reads)
            if missing_reads:
        ...
        for reference in payload.get("evidence_references") or ():
            ...
            if not any(_scope_allows(allowed, path) for allowed in allowed_evidence_paths):
    ...
    ingester_class._validate_report_correlation = validate_report_correlation
...
def _reserve_automatic_hq_wake_budget(orchestrator: WorkerGitHubOrchestrator, run_id: str):
    ...
    try:
        ...
        command_center.trip_safety_pause(
            reason=(
                "An automatic HQ review wake reserved a global send attempt, but the reservation "
        ...
        raise WorkerRuntimeError(
            "Automatic HQ review send-budget evidence could not be persisted before transport."
        ) from exc
    return decision
...
def _install_orchestrator_relay() -> None:
    ...
    worker_github_orchestrator_runtime._hq_review_path = _canonical_hq_review_path
    ...
    def send_hq_wake(
        ...
        if row is None or str(row["result_state"] or "") != "REPORT_VALIDATED":
            return
        if self._artifact_exists(_canonical_hq_review_path(row, run_id)):
        ...
        if run_lock.locked():
            return
        if "hq_wake_claimed_at" in row.keys() and row["hq_wake_claimed_at"] is not None:
            guarded_send_hq_wake(self, run_id, advisory_id)
            return
        ...
        try:
            ...
            self.operations.command_center.trip_safety_pause(
                reason=(
                    "Automatic owning-HQ wake failed or became uncertain after a global send "
                    "attempt was reserved. Manual review is required before retry."
                ),
                affected_run_id=run_id,
        ...
        if current is not None and str(current["hq_wake_state"] or "") == "HQ_WAKE_SUBMITTED":
            if not bool(current["hq_wake_returned_to_source"]):
                self.operations.command_center.trip_safety_pause(
                    reason="Automatic owning-HQ wake did not verify return to the source chat.",
                    affected_run_id=run_id,
                    trigger="hq_review_orchestrator",
                )
                raise WorkerRuntimeError(
                    "Automatic owning-HQ wake did not verify return to the source chat."
                )

    orchestrator_class._ingest_hq_review_if_present = ingest_hq_review_if_present
    orchestrator_class._ingest_rob_validation_if_present = ingest_rob_validation_if_present
