"""Core deterministic Package E report rejection and correction-only repair coordination.

The immutable Worker outbox remains Git evidence and ``execution_history`` remains the one runtime
ledger. A rejected report may derive exactly one narrow next-attempt contract. The derived contract
adds the rejected report and machine rejection artifact as read-only evidence inputs, while the only
write authority is the new immutable report attempt.
"""
...
from .worker_result_ingester import (
    GitArtifactEvidence,
...
REPAIR_PENDING = "REPORT_REPAIR_PENDING"
REPAIR_ACCEPTED = "REPORT_REPAIR_ACCEPTED"
REPAIR_EXHAUSTED = "REPORT_REPAIR_EXHAUSTED"
...
class WorkerReportRepairWake:
    ...
    def idempotency_key(self) -> str:
        return f"report-repair:{self.worker_id}:{self.run_id}:{self.next_report_attempt}"
...
class WorkerReportRejectionReceipt:
    """Evidence that one malformed report was rejected and repair was queued or exhausted."""
...
def _error_code(message: str) -> str:
    ...
    if any(token in lowered for token in ("git", "blob", "commit", "evidence")):
...
def render_repair_instruction(
    ...
    *,
    ...
    return (
        ...
        f"Read the rejected report at `{rejected_report_path}` and the deterministic rejection "
        f"artifact at `{rejection_path}`. The rejected report was attempt {rejected_attempt}. "
        "Do not repeat or broaden the underlying work. Correct only the report artifact and create "
        f"exactly one new immutable attempt at `{corrected_report_path}`. Preserve the same run, "
        "Worker, task revision, procedure, authorization source, verification mode, substantive "
        "findings, and source evidence unless the rejection artifact identifies a report-correlation "
        "defect that must be corrected. Record both repair-input paths in actual read scopes and "
        "evidence references. Do not overwrite, delete, edit the source advisory, change lifecycle, "
        "re-execute work, expand scope, or perform any unrelated write or external action. "
...
class WorkerResultRepairCoordinator:
    ...
    def _repair_inputs(self, advisory: ExecutionReadyAdvisory) -> tuple[str, str]:
        row = self._row(advisory.run_id)
        rejected_report_path = str(row["rejected_report_path"] or "").strip()
        rejection_path = str(row["rejection_path"] or "").strip()
        if not rejected_report_path or not rejection_path:
            raise WorkerRuntimeError("Repair attempt lacks preserved rejection input paths.")
        return rejected_report_path, rejection_path

    def _derived_advisory(
        self, advisory: ExecutionReadyAdvisory, attempt: int
        ...
        if attempt == original.attempt:
            return replace(
                advisory,
                requested_write_scopes=(contract.result_path,),
                result_contract=contract,
            )
        rejected_report_path, rejection_path = self._repair_inputs(advisory)
        repair_reads = tuple(
            dict.fromkeys(
                (
        ...
        return replace(
            advisory,
            source_references=repair_sources,
            requested_read_scopes=repair_reads,
            requested_write_scopes=(contract.result_path,),
            result_contract=contract,
        )
    ...
    def _repair_wake(
        ...
        *,
        ...
        contract = corrected.result_contract
        if contract is None:
            raise WorkerRuntimeError("Derived repair assignment lost its result contract.")
        corrected_path = contract.result_path
        return WorkerReportRepairWake(
            wrapper_id=(
                f"REPAIR-{advisory.advisory_id}-R"
                f"{advisory.advisory_revision}-A{next_attempt}"
            ),
    ...
    def prepare_repair(
        ...
        rejected_path = str(row["report_path"] or "").strip()
        rejected_checksum = str(row["report_checksum"] or "").strip()
        ...
        wake = None
        if not exhausted:
            # Persist repair inputs before deriving the correction-only attempt contract.
            with self._connect() as connection:
                connection.execute(
                    """
                    UPDATE execution_history SET
                        rejected_report_path = ?, rejected_report_checksum = ?,
        ...
        repair_state = REPAIR_EXHAUSTED if exhausted else REPAIR_PENDING
        wake_json = (
        ...
        with self._connect() as connection:
            rows = connection.execute(
                ...
                SELECT id, run_id, worker_id, task_id, task_revision,
                       result_state, report_attempt, rejected_report_path,
                       rejected_report_checksum, rejection_path,
    ...
    def status(self, limit: int = 100) -> dict[str, object]:
        ...
        return {
            "summary": {
                "total": len(records),
                "pending": sum(item.get("repair_state") == REPAIR_PENDING for item in records),
                "accepted": sum(item.get("repair_state") == REPAIR_ACCEPTED for item in records),
                "exhausted": sum(item.get("repair_state") == REPAIR_EXHAUSTED for item in records),
            },
            "records": records,
        }
