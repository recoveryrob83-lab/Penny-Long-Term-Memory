...
class SyntheticRepairCourierReceipt:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "rejected_report_path": self.rejected_report_path,
...
def _report_payload(
    ...
    *,
    ...
    repair_inputs: dict[str, str] | None = None,
) -> dict[str, object]:
    report_path = REPORT_1 if attempt == 1 else REPORT_2
    repair_inputs = repair_inputs or {}
    evidence = [
        *(f"{path}@{blob_shas[path]}" for path in TARGETS),
        *(f"{path}@{blob_sha}" for path, blob_sha in repair_inputs.items()),
        f"{report_path}@preflight:not-found",
    ]
    reads = [*TARGETS, *repair_inputs.keys(), report_path]
    ...
    payload.update(
        {
            ...
            "actual_action_attempted": (
                "Reused the completed synthetic inspection, read the rejected report and rejection "
                "artifact, and corrected only the report artifact."
                if attempt > 1
...
def _synthetic_wake(wake: WorkerReportRepairWake) -> SyntheticRepairCourierReceipt:
    ...
    required_markers = (
        "Do not repeat",
        wake.rejected_report_path,
        wake.rejection_path,
        wake.corrected_report_path,
    )
    if not all(marker in wake.instruction for marker in required_markers):
        raise WorkerRuntimeError("Synthetic repair wake lacks correction-only evidence pointers.")
...
def run_synthetic_repair_pilot(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "rejection_blob_sha": str(row["rejection_blob_sha"]),
