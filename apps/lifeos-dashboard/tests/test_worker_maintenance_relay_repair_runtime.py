...
def test_scope_containment_accepts_descendants_and_read_only_annotations() -> None:
    assert _normalized_scope("apps (read-only schema evidence)") == "apps"
    assert _scope_allows("memory", "memory/HQ_NAMING_STANDARD.md")
    assert _scope_allows("apps", "apps (read-only schema evidence)")
    assert not _scope_allows("memory", "coordination/ADVISORY_INDEX.md")
    assert _unauthorized_scopes(
        ("memory", "coordination"),
        ("memory/01_SESSION_HANDOFF.md", "coordination/ADVISORY_INDEX.md"),
    ) == []
...
def test_evidence_reference_parser_accepts_canonical_git_witnesses() -> None:
    assert _parse_evidence_reference(f"memory/file.md@{BLOB_SHA}") == (
        "memory/file.md",
        None,
        BLOB_SHA,
    )
    assert _parse_evidence_reference(f"memory/file.md@blob:{BLOB_SHA}") == (
        "memory/file.md",
        None,
        BLOB_SHA,
    )
    ...
    ) == ("memory/file.md", COMMIT_SHA, BLOB_SHA)
    assert _parse_evidence_reference("projects/result.json@preflight:not-found") == (
        "projects/result.json",
        None,
        None,
    )
    with pytest.raises(WorkerRuntimeError, match="witness is invalid"):
        _parse_evidence_reference("memory/file.md@sha256:example")


def test_maintenance_artifact_paths_use_life_logistics_project_root() -> None:
    ...
    assert _canonical_hq_review_path(row, run_id) == (
        ...
        "RUN-ADV-20260726-053-R1/hq-review-001.json"
    )
    assert _canonical_rob_validation_path(row, run_id) == (
        "projects/life-logistics-hq/worker-results/maintenance_worker/"
        "RUN-ADV-20260726-053-R1/rob-validation-001.json"
    )


def test_report_correlation_accepts_broad_scopes_and_commit_blob_evidence() -> None:
    ...
    advisory = SimpleNamespace(
        result_contract=SimpleNamespace(attempt=1, result_path=result_path),
        requested_read_scopes=("memory", "coordination", "projects", "apps", "workers"),
        requested_write_scopes=("memory", "coordination", "projects", result_path),
    ...
    payload = {
        ...
        "actual_write_scopes": ["memory/HQ_NAMING_STANDARD.md", result_path],
        "actual_tools": ["GitHub"],
    ...
    ingester._validate_report_correlation(advisory, profile, payload)


class _Decision:
    def __init__(self, *, reserved: bool, reason: str = "") -> None:
...
def test_automatic_hq_wake_reserves_shared_budget_and_persists_evidence() -> None:
    command_center = _CommandCenter(_Decision(reserved=True))
    orchestrator = SimpleNamespace(
        operations=SimpleNamespace(command_center=command_center)
    )

    decision = _reserve_automatic_hq_wake_budget(orchestrator, "RUN-1")

    assert decision.reserved is True
    assert command_center.reservations == [("hq_review_wake", "RUN-1")]
    assert command_center.evidence == [("RUN-1", decision)]
    assert command_center.pauses == []


def test_automatic_hq_wake_budget_refusal_trips_shared_pause() -> None:
    command_center = _CommandCenter(_Decision(reserved=False, reason="Budget exhausted."))
    orchestrator = SimpleNamespace(
        operations=SimpleNamespace(command_center=command_center)
    )

    with pytest.raises(WorkerRuntimeError, match="Budget exhausted"):
        _reserve_automatic_hq_wake_budget(orchestrator, "RUN-2")
