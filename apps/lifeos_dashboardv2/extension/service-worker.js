...
async function poll() {
  ...
  try {
    const result = await chrome.tabs.sendMessage(tab.id, {type:'dispatch', wake:begun.wake_payload, routeUrl:local.routeUrl});
  ...
  } catch (_) { await call(`/commands/${encodeURIComponent(begun.command_id)}/uncertain`, {method:'POST', body:JSON.stringify({note:'Navigation or extension communication became indeterminate after dispatch began.'})}).catch(()=>{}); }
}
