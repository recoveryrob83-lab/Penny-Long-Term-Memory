...
- During routine boots and syncs, compare current working context with canonical department files and correct drift directly when authorized.
- Do not route routine local maintenance through Logistics or formal advisories.
...
- Logistics maintains shared operational infrastructure, global hygiene, and cross-project audits.
...
Windows ChatGPT Classic department-boot automation is operational across all seven HQs.
...
Read scheduled-task files only when Rob asks about scheduled tasks, Logistics HQ is doing system review, Main Assistant needs morning-report notes, or a department is told to check its memo inbox.
...
Read the Advisory Index when Rob asks for advisory status, Logistics HQ is doing review, Main Assistant is preparing a full operations report, a project chat is being recreated after connector problems, or Rob routes a department to a specific advisory.

Read a specific source department board only when the Advisory Index points to a relevant open advisory or Rob names the board or advisory.
...
- Legacy Virtual Assistant Business: `projects/virtual-assistant-business/SESSION_HANDOFF.md` redirects to Office Leaks HQ and should be used only for historical context.
...
Read the project's `README.md`, `status.md`, and `open_loops.md` when they exist or are maintained. Do not treat an intentionally absent optional file as a boot failure.
...
- Build working context from repository files.
- For specialist chats, summarize global context briefly and then focus on the project handoff and department identity.
- For Logistics HQ, focus on system-level state, active projects, open loops, advisory status, dashboard and automation state, worker-layer status, and role clarity.
...
Logistics HQ maintains shared operational infrastructure, global hygiene, and cross-project audits.
