...
| Active | Normal | Collapsed LifeOS project recovery | Pull the bounded recovery patch, run the focused tests, deliberately fold the exact `Life OS` project, and execute one draft-only automation into a known empty composer | GitHub implementation detects only the exact collapsed project control in the left sidebar, expands it once, verifies that project chat links became visible, then preserves the existing exact-chat and single-`Show more` navigation path. Expanded, ambiguous, or unverifiable projects are not clicked |
| Active | Normal | Scheduler production preflight | Validate folded-project recovery, then implement the approved pause-on-failure / pause-on-overdue policy and decide the exact one-time resume behavior | Scheduler ledger transport, successful cleanup, empty-startup compaction, overdue visibility, and bounded recurrence are live-validated. Active rows become `OVERDUE` in the cloud Sheet after a five-minute grace period; the current local overdue catch-up behavior remains unchanged until the pause policy is implemented and validated |
