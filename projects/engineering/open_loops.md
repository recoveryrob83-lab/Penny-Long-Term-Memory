...
| Active | Normal | Scheduler production preflight | Pull the production-policy patch, run the focused scheduler suites, restart the dashboard, then validate overdue pause-without-catch-up, failed-run pause, recurring Resume rearm, and expired one-time Resume refusal | Folded-project recovery is live-validated. The production scheduler policy now shares the Sheet's strict five-minute grace threshold, pauses stale definitions without fake run history, pauses failed or nontransiently refused runs, advances only successful recurrence, recalculates recurring Resume from the current time, and requires expired one-time definitions to be edited into the future |
...
| Paused | Low | Engineering HQ Daily Sync | Keep paused until the scheduler production policy is live-validated and Rob explicitly resumes it | Scheduling exists, but this specific unattended production task is not yet reauthorized |
