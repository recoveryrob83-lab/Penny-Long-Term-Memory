...
|---|---|---|---|---|
| Active | High | Retired-title compatibility runtime validation | Run one draft-only invocation using `Main Assistant HQ` or `Logistics HQ` and confirm it translates to the canonical chat while preserving exact-destination verification | Canonical draft-only checks passed for Chief of Staff HQ and Life OS Maintenance HQ. Existing schedules and run history display the correct canonical names without database migration |
