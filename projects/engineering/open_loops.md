...
| Active | High | Clipboard write-verification repair validation | Pull the latest dashboard code, restart it, run the focused tests, then repeat one draft-only Engineering custom prompt with the prior clipboard set to a distinctive short value | Raw stdout/stderr proved the window, exact destination, and composer were verified. The intended prompt was restored off the clipboard before asynchronous paste consumption and only the prior clipboard value `i` appeared. The dashboard also ignored `LIFEOS_RESULT_CODE=write_verification_failed` and misclassified the failure as ChatGPT unavailable. Bounded repairs are committed; local Windows validation remains pending |
