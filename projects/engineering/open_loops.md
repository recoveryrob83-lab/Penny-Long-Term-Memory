...
| Active | Normal | Bounded five-minute recurrence validation | Pull the latest dashboard code, restart it, run the focused tests, then create one `Debug: every 5 minutes (2 drafts)` Engineering schedule and clear the first draft before the second attempt | The new cadence is draft-only, uses no database migration, schedules exactly one five-minute repeat, then auto-disables after the second completed attempt. Verify both history entries, the 1/2 UI state, the final Completed state, and no third run |
| Open | Normal | Scheduler production preflight | Define health visibility, missed-run policy, failed one-time retry behavior, and unattended operating requirements | Overdue one-time work catches up immediately after restart, records the result, disables after one attempt, and does not retry automatically. Clipboard-safe restart execution now passes. Decide whether the explicit failed state plus manual retry is sufficient before unattended use |
