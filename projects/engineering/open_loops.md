...
| Active | Normal | Cloud scheduler ledger validation | Verify one-row create/edit/pause/run/delete behavior plus offline `OVERDUE` calculation | The no-billing Apps Script endpoint is deployed, the shared secret and local endpoint are configured, and Rob confirmed the dashboard displays `Scheduler running · Ledger synced`. The native `LifeOS Scheduler Ledger` remains a cloud-visible monitoring mirror with one row per schedule and no worker or Penny writeback requirement |
