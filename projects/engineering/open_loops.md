...
| Active | High | Post-restart ChatGPT Classic recovery | With the dashboard already running and Engineering HQ composer cleared, run one immediate draft-only custom prompt to isolate whether exact window reacquisition now succeeds; inspect the actual accessible top-level window title before changing selectors | The overdue scheduler persisted across dashboard shutdown and fired once about 15–20 seconds after restart, but desktop automation reported ChatGPT Classic unavailable despite the app being open. Rob also observed a stray `i` placed before validation. Do not broaden window matching or add retries until the failure is reproduced and bounded |
