...
| Active | Normal | Cloud scheduler ledger validation | Deploy the bound Apps Script, configure the shared secret and local endpoint URL, rerun the focused suite, and verify one-row create/edit/pause/run/delete behavior plus offline `OVERDUE` calculation | The native `LifeOS Scheduler Ledger` Sheet and bounded dashboard integration are implemented. The service-account transport was replaced by a no-billing Apps Script endpoint. Command Center SQLite remains the local authority; the Sheet is a cloud-visible monitoring mirror with one row per schedule and no worker or Penny writeback requirement. The earlier focused suite passed 20 tests before the transport replacement |
