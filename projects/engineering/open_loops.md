...
| Active | Normal | Cloud scheduler ledger validation | Pull the startup-compaction patch, redeploy the Apps Script as a new version, restart the dashboard, and confirm existing blank Sheet rows are deleted even when zero schedule definitions remain | Rob confirmed completed definitions clear from the dashboard and their Sheet cells clear, but blank rows remained. Root cause: compaction ran only inside schedule upsert/remove requests, so an empty dashboard startup sent no request. The dashboard now sends an explicit authenticated `compact` action before publishing any remaining schedules |
| Open | Normal | Scheduler production preflight | Complete ledger and folded-project validation, then implement the approved pause-on-failure / pause-on-overdue operating policy and decide the exact one-time resume behavior | Missed-run visibility no longer requires a large automatic retry policy. Active rows become `OVERDUE` in the cloud Sheet after a five-minute grace period; the current local overdue catch-up behavior remains unchanged until the pause policy is implemented and validated |
...
| Open | Normal | Connector health and retry policy | Define read/write health states, bounded retry rules, stop/backoff triggers, and degraded-mode fallback | Dashboard caches, guarded Git sync, desktop automation safety gates, and the scheduler Sheet mirror provide concrete examples |
...
| Paused | Low | Engineering HQ Daily Sync | Keep paused until scheduler ledger, pause policy, and folded-project recovery are validated and Rob explicitly resumes it | Scheduling exists, but this specific unattended production task is not yet reauthorized |

## Waiting / Deferred

| Status | Item | Owner / Trigger | Notes |
|---|---|---|---|
| Waiting | Collapsed LifeOS project recovery | After scheduler ledger validation | Add bounded exact-project detection, expand once when folded, verify the chat region became visible, then preserve exact-chat navigation and stop safely on uncertainty |
...
| 2026-07-18 | Overdue one-time scheduler catch-up validation | A draft-only Engineering schedule persisted while the dashboard was stopped, became overdue, fired once about 15–20 seconds after restart, recorded its result, disabled itself, and did not retry automatically |
