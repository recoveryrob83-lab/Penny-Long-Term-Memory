...
| Active | Normal | Cloud scheduler ledger validation | Pull the completed-cleanup patch, rerun the focused suite, verify the final scheduled test, then use `Clear completed` and confirm successful definitions disappear from both the dashboard and Sheet while run history remains | The no-billing Apps Script endpoint is deployed, the shared secret and local endpoint are configured, and Rob confirmed the dashboard displays `Scheduler running · Ledger synced`. The native `LifeOS Scheduler Ledger` remains a cloud-visible monitoring mirror with one row per schedule and no worker or Penny writeback requirement |
