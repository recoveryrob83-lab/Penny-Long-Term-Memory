...
| Open | Normal | Scheduler production preflight | Define health visibility, missed-run policy, failed one-time retry behavior, collision spacing, production interval options, and unattended operating requirements | Overdue one-time work catches up immediately after restart, records the result, disables after one attempt, and does not retry automatically. Clipboard-safe restart execution and a bounded two-run recurrence both pass. Production cadence and run-count ideas remain unpromoted until Rob authorizes design work |
