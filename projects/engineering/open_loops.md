...
| Open | Connector health and retry policy | Define read/write health states, bounded retry rules, stop/backoff triggers, and degraded-mode fallback | Dashboard caches, guarded Git sync, desktop automation safety gates, and Command Center structured results provide concrete examples |
...
| Open | Engineering HQ Daily Sync | Keep paused until scheduled execution architecture is more reliable and Rob explicitly resumes it | Command Center Phase 1 is manual Run Now only and does not authorize unattended scheduling |

## Waiting / Deferred

| Status | Item | Owner / Trigger | Notes |
|---|---|---|---|
| Waiting | Command Center one-time scheduling | Phase 1 manual path proves dashboard-to-runner safety, locking, structured results, and restart behavior | Do not implement in Phase 1 |
| Waiting | Command Center recurring scheduling | One-time scheduling proves reliable and Rob explicitly authorizes recurring jobs | Daily, weekly, and every-X-hours schedules remain later-phase work |
| Waiting | Automatic ChatGPT launch and richer recovery | Demonstrated need after manual and scheduled jobs prove stable | Screenshots, notifications, preflight checks, and diagnostic bundles remain later phases |
...
|---|---|---|
| 2026-07-17 | Automation Command Center plan | Defined eight destinations, manual Phase 1, safety contract, job model, activity logging, and phased scheduling architecture |
...
| 2026-07-17 | Logistics boot-path and launcher alignment | Corrected `projects/life-logistics` to `projects/life-logistics-hq`, aligned canonical HQ labels, and routed the legacy wrapper through the canonical launcher |
...
- Revisit unattended department scheduling only after Command Center Phase 1 and later scheduling phases prove safe and Rob explicitly authorizes it.
