...
|---|---|---|---|
| Open | Live Trello dashboard adapter | Define a read-only local or API-backed source path for Now, top Next, selected Waiting, and optional Inbox count | Begin only after Rob pulls and verifies the merged live GitHub version locally |
| Open | Dashboard live-mode review | Confirm the GitHub pulse, notebook resurfacing, open-loop summaries, and responsive layout remain useful with real repository data | Capture noise, missing context, stale parsing, or scan-path problems before adding another source |
...
| 2026-07-17 | Dashboard live-adapter verification | Reconstructed branch package passed 5 tests covering sample mode, local checkout detection, advisory parsing, priority-loop parsing, notebook discovery, and recent activity normalization |
