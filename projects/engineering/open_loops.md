...
| Active | Normal | Completed recurrence placement UI validation | Pull the latest dashboard code, restart it, and confirm the completed two-run debug definition no longer appears under the default Upcoming view while both executions remain in Execution Record | The UI previously labeled the definition `Completed` but the default scheduler filter only excluded finished one-time jobs. The bounded fix hides completed debug definitions from Scheduled and Paused views, preserves them under All definitions, and does not alter execution history |
