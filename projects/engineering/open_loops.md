...
| Active | High | Post-restart ChatGPT Classic startup recovery | Design a bounded startup-readiness grace and exact-window reacquisition path before overdue jobs execute, then revalidate draft-first | The same Engineering custom draft succeeded immediately once the dashboard was fully running. This isolates the earlier failure to post-restart readiness timing rather than destination navigation, clipboard verification, or ordinary attended automation. Preserve exact-window and exact-chat verification; do not add broad title matching or uncontrolled retries |
...
| Open | Normal | Scheduler production preflight | Define health visibility, missed-run policy, failed one-time retry behavior, and unattended operating requirements | Overdue one-time work currently catches up immediately after restart, records the result, disables after one attempt even when execution fails, and does not retry automatically. Decide whether the explicit failed state plus manual retry is sufficient before unattended use |
...
|---|---|---|
| 2026-07-18 | Attended post-restart automation isolation | With the dashboard fully running, the same immediate Engineering custom draft completed correctly, isolating the earlier overdue-run failure to startup readiness timing rather than ordinary desktop automation |
