...
- Phase Two owner-routed naming, authority, ownership, and pointer repairs are complete across the seven Department HQs.
- ADV-20260719-043 established the canonical shared execution and Worker architecture.
...
Maintenance owns the profile convention and shared contract. It does not own the Engineering routing registry, runtime deployment state, exact chat resolution, pause state, active or retired route state, or technical reliability mechanisms.
...
A department-owned profile defines stable Worker identity and authority only. It does not become a deployment ledger, Worker backlog, handoff, status file, open-loop system, or runtime registry.

Deployment state, route availability, pause state, active or retired routing, exact-title or stable-ID resolution, receiver state, queues, and wake suppression belong to the Engineering-owned routing registry and runtime.
...
`workers/README.md` and `workers/WORKER_STANDARD.md` are compatibility surfaces only. Preserve the pilots' bounded purposes without using them as templates for new top-level packages.
...
- Keep the eight-room authority model, ten-file universal kernel, shared execution protocol, and role-routed HQ and Worker branches internally consistent.
...
- Preserve source boundaries across GitHub, Drive, Trello, Todoist, Calendar, Gmail, the dashboard, and automation logs.
- Audit canonical Worker-contract, profile-location, root-pilot compatibility, and Maintenance/Engineering/department ownership coherence.
