...
Maintenance protects the LifeOS operating system through global GitHub coherence, universal boot integrity, shared operating rules, system open-loop hygiene, advisory infrastructure, repository structure, audits, source-boundary enforcement, Worker-profile conventions, canonical shared execution contracts, Project Source publication discipline, and reconciliation between the Drive Chief's Manual, Project Instructions, Project Sources, and GitHub implementation.

`Chief_of_Staff_HQ` is Rob's primary point of contact, personal-assistant headquarters, daily-operations desk, Hub chair, routing desk, report receiver, watcher destination owner, and follow-through coordinator.
...
- Wave 0B created no non-Engineering Worker, route, permission, schedule, dispatch authority, durable-write authority, or rollout approval.
...
## Maintenance Worker Prerequisites on Main
...
- Initial task scope: read-only audits, Sync, drift detection, pointer checks, ownership checks, naming checks, source-boundary checks, advisory-index checks, and handbook-source checks
...
Canonical new profile location:
...
- LifeOS Dashboard and automation logs: transport, diagnostics, run state, and bounded local controls.
...
Detailed financial, medical, business, personal, credential, customer, and source-system records remain outside GitHub.

## Advisory State

Use `coordination/ADVISORY_INDEX.md` as the live source of truth. Do not cache a static advisory list in this handoff.
...
- Keep the eight-room authority model, universal kernel, shared execution protocol, and role-routed HQ and Worker branches internally consistent.
- Preserve exact canonical room and Worker names in current text while retaining legitimate historical evidence and stable paths.
...
- Keep fully unattended Windows production unapproved until Rob authorizes a proven scope.
- Preserve the paused `Engineering_HQ Daily Sync` state.
- Preserve `archive/projects/virtual-assistant-business/` as historical evidence and active Office Leaks state at `projects/office-leaks-consulting/`.
